...
// [GET] /api/v1/city/search?name=
const searchForCity: RequestHandler = async (req: Request, res: Response) => {
  const { name } = req.query;

  const requiredName = name?.toString() || '';

  const response = await cityService.searchForCity(requiredName);
